...
export default function ViewUsersPage({ params }: { params: { id: string, role: string } }) {
    // Params
        ...
        <form className="flex flex-col w-full h-full gap-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            <section className="flex w-full h-full gap-6 border-2 rounded-2xl p-4 border-[#D4D4D8]">
                ...
                <div className="flex flex-col gap-4 w-full">
                    ...
                    {tab === "general" && (
                        <FormUserInfo data={user} role={data ?? null} register={register} errors={errors} status="view" />
                    )}

                    {tab === "account" && (
                        <FormAccountInfo data={user} register={register} errors={errors} status="view" />
                    )}
                </div>
            </section>
        </form>
